...
- WS1 single-card batch-invariant operator parity:
  - RMSNorm;
  ...
  - ratio / KL and GRPO loss fragments where already in the forward / loss
    validation chain.
- WS2 distributed and cross-config invariance:
...
reusing Hopper-only assumptions.
...
Blackwell-only tolerance story.
...
- [ ] Ensure `scripts/check_operator.py`, gtest specs, and the cross-config
      comparison harness can select Blackwell candidates by backend key.
- [ ] Register the full WS1 forward-chain operator set, not only `logp` and
      `linear_logp`: RMSNorm, matmul, attention, KV-cache attention, RoPE,
      SwiGLU / SiLU, embedding, LM head, ratio / KL, and GRPO loss.
...
Goal: before testing communication, make sure each WS1 operator has a
Blackwell-safe fallback and, when available, a validated Blackwell optimized
candidate.
...
- [ ] Reuse or introduce `ShardingSpec` / `Placement` for TP / SP / CP paths:
      `Shard(dim)`, `Replicate()`, and `Partial()`.
- [ ] Reuse or introduce `ReductionSpec` for every distributed operator:
...
- [ ] `[Blackwell][platform] Add architecture probe, runtime facts report, and CUDA backend keys`
- [ ] `[Blackwell][build] Add sm100/sm120/PTX build and compatibility checks`
- [ ] `[Blackwell][dispatch] Prevent silent SM90-only kernel selection on Blackwell`
- [ ] `[Blackwell][tests] Extend operator gtest and dlogp reports for Blackwell candidates`
- [ ] `[Blackwell][logp] Validate selected logprob fallback and Blackwell candidate`
- [ ] `[Blackwell][linear_logp] Validate large-vocab linear_logp and LM-head path`
- [ ] `[Blackwell][attention] Validate standard, varlen, LSE-export, and KV-cache attention`
- [ ] `[Blackwell][matmul] Scope deterministic GEMM / split-K policy`
- [ ] `[Blackwell][rms_norm] Validate fused RMSNorm or keep explicit fallback`
- [ ] `[Blackwell][elementwise] Audit RoPE, SiLU, SwiGLU, and embedding`
- [ ] `[Blackwell][loss] Validate ratio-KL and GRPO loss forward/backward`
- [ ] `[Blackwell][distributed] Add TP/SP/CP reduction contract and deterministic fallback`
- [ ] `[Blackwell][vime] Add vime fallback smoke and one-op-at-a-time enablement report`
- [ ] `[Blackwell][docs] Publish build, fallback, validation, and benchmark notes`
